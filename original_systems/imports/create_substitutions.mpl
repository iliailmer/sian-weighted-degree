...
GetSubsTableFreq := proc(sigma, {exponent:=2})
  ...
  rhs_terms_full,monoms,constants_to_sub, counting_table_const, min_count, rhs_term, opposites, i, j, substitutions, new_subs,indets_, term,  each, elem:
  ...
  rhs_terms_full := map(f->op(rhs(f)), sigma):
  y_functions_rhs :=  foldl(`union`, op(convert(map(x->indets(rhs(x)) minus {t}, select(f->not is_diff(lhs(f)), sigma)), set)));

  monoms := select(x->nops(x)=2, map(x->indets(x) minus {t}, rhs_terms_full)); 

  monoms := select(x-> nops(x intersect y_functions_rhs)>0, monoms); 
  ...
  substitutions := table(map(f->parse(StringTools[Split](convert(lhs(f), string), "_")[1])=exponent, select(f->evalb(rhs(f)<=min_count), [entries(counting_table_fun, `pairs`)]))):
...
GetSubsTable := proc(sigma, {exponent:=2, min_level:=1, strict:=false, use_functions_only:=true})
  ...
  local system_vars, vt, vtc, max_possible, rhs_terms, rhs_terms_full, 
   opposites, i, j, substitutions, new_subs, each, elem, y_functions_rhs, monoms, constants_to_sub:
  ...
  if strict = false then
    ...
    max_possible:=max(map(rhs, [entries(vt, `pairs`)]));
    substitutions := table(map(f->parse(StringTools[Split](convert(lhs(f), string), "_")[1])=exponent, select(f->evalb(rhs(f)=max_possible), [entries(vt, `pairs`)]))): 
  fi:
  # take max possible leve
