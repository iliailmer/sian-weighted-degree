...
#===============================================================================
GetPolySystem := proc(system_ODEs, params_to_assess, {sub_transc:=true, count_solutions:=true, p := 0.99, infolevel := 2, method := 2, num_nodes := 6, max_comb:=200})
#===============================================================================
 local i, j, k, n, m, s, all_params, all_vars, eqs, Q, X, Y, poly, d0, D1,
  ...
  if sub_transc and numelems(alg_indep)<>0 then
    ...
    X_eq, Y_eq, Et, _, x_vars, y_vars, mu, beta, Q, d0 := PreprocessODE(sigma_new, GetParameters(sigma_new)):
