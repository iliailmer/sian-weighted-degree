...
array=(original_hpv_mf_group4 new_ssaair original_pharm original_hpv_mf_group1_subs original_hpv_mf_group4 new_seiqrdc new-goodwin-osc new_seir original_nfkb new_seirp new_seir2) #new_siraqj new_seiajrc original_hiv2 new_qwwc new_ssaair new-goodwin-osc new_seir) #original_lv original_oral_glucose original_sir_r0 original_sirsforced original_slowfast original_treatment original_tumor new-akt new-cd8-cells new-goodwin-osc original_biohydrogenation original_hiv original_hiv2 new_saird new_seiddrf new_seir new_seirc new_seirp new_qwwc new_seiajrc new_seiqrdc new_siar new_sidarthe new_siraqj new_sircd new_sir_denom new_sliqr new_ssaair original_nfkb new_seirahd new_mapk_2)
...
for each in "${array[@]}"; do
    ...
    for d in $each/*with_trb_zero_char.m; do
        outname=(${d//"/"/ })
